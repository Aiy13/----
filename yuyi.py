...
class Yuyi:
    ...
    def analyze_assignment(self, stmt):
        ...
        for symbol_entry in self.symbol_table:
            if symbol_entry['name'] == var_name:
                symbol_entry['value'] = expr_value
                break
    ...
    def analyze_function_declaration(self, stmt):
        ...
        # 添加参数到函数作用域
        for param in params:
            self.add_symbol(param['name'], param['type'], None)
    ...
    def analyze_function_call(self, expr):
        ...
        for i, (arg, (param_name, param_type)) in enumerate(zip(args, func_info['params'])):
            ...
            if arg_type != param_type:
                self.errors.append(
                    f"第{expr['line']}行：函数 '{func_name}' 第{i + 1}个参数类型不匹配，期望 {param_type}，得到 {arg_type}")
        ...
        if func_info['return_type'] == 'void':
            ...
            if not stmt.get('value'):
                ...
                if return_type != func_info['return_type']:
                    self.errors.append(
                        f"第{stmt['line']}行：返回类型不匹配，期望 {func_info['return_type']}，得到 {return_type}")
    ...
    def get_expression_value(self, expr):
        ...
        if expr['type'] == 'number':
            ...
            # For simplicity, return None for complex expressions
            # Could be extended to evaluate constant expressions
            return None
        elif expr['type'] == 'unary_expression':
            return None
